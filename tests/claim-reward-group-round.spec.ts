...
describe("claimRewardGroupRound", () => {
  ...
  before(async () => {
    ...
    r = await program.account.round.fetch(roundPda);
    let remainingAccountsStartRound = [];
    for (let groupId = 1; groupId <= r.totalGroups.toNumber(); groupId++) {
      const groupAssetPda = deriveGroupAssetPda(
        program.programId,
        roundPda,
      ...
      remainingAccountsStartRound.push({
    ...
    try {
      await program.methods
        ...
        .remainingAccounts(remainingAccountsStartRound)
        .signers([keeper])
        .rpc();
    } catch (e: any) {
      throw e;
    ...
    // finalize end groups
    r = await program.account.round.fetch(roundPda);
    let remainingAccountsEndGroups = [];
    for (let groupId = 1; groupId <= r.totalGroups.toNumber(); groupId++) {
      const groupAssetPda = deriveGroupAssetPda(
        program.programId,
        roundPda,
      ...
      remainingAccountsEndGroups.push({
        pubkey: groupAssetPda,
        isSigner: false,
        isWritable: false,
      });
    ...
    try {
      await program.methods
        ...
        .remainingAccounts(remainingAccountsEndGroups)
        .signers([keeper])
        .rpc();
    } catch (e: any) {
      throw e;
    ...
    // settle group round
    r = await program.account.round.fetch(roundPda);
    const remainingAccountsSettleGroupRound = [];
    for (let betId = 1; betId <= r.totalBets.toNumber(); betId++) {
      const betPda = deriveBetPda(
        program.programId,
        roundPda,
        new anchor.BN(betId)
      );
      remainingAccountsSettleGroupRound.push({
        pubkey: betPda,
        isSigner: false,
        isWritable: true,
      });
    ...
    try {
      await program.methods
        ...
        .remainingAccounts(remainingAccountsSettleGroupRound)
        .signers([keeper])
        .rpc();
    } catch (e: any) {
      throw e;
  ...
  it("happy path", async () => {
    ...
    for (let betId = 1; betId <= r.totalBets.toNumber(); betId++) {
      ...
      expect(Number(balanceAfter.value.amount)).to.greaterThan(
